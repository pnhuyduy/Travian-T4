...
window.addEvent("domready", function (){
	...
	for (i=0;i<ancs.length;i++){
		ancs[i].addEventListener('click', function(){
				...
				if (tmphref!='' && tmphref!=null ) {
					if (tmptarg=='_blank') { window.open(tmphref);} else {window.location.href=tmphref;}
				}
			}
		);
	}
	...
	for (i=0;i<areas.length;i++){
		areas[i].addEventListener('click', function(){var tmphref = this.getAttribute('href'); this.removeAttribute('href'); if (tmphref!='' && tmphref!=null  ) window.location.href=tmphref;});
	}
});
